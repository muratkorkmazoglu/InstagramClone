...
class UniversalImageLoader(var mContext: Context?) {
    ...
    val config: ImageLoaderConfiguration
        get() {
            ...
            return ImageLoaderConfiguration.Builder(mContext).defaultDisplayImageOptions(defaultOptions)
                .memoryCache(WeakMemoryCache()).diskCacheSize(100 * 1024 * 1024).build();
        }

    companion object {
        private val defaultImage = R.drawable.ic_profile;

        fun setImage(imgUrl: String, imageView: ImageView, mProgressBar: ProgressBar?, ilkKısım: String?) {
